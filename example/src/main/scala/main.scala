...
object Demo {
  ...
  val nav = st.NavigationGroup[Int](
    ...
    _.children := Seq(
      st.NavigationItem(1)(_.label := "a"),
      st.NavigationItem(2)(_.label := "b")
    )
  )
  ...
  def root() = div(
    ...
    div(
      ...
      child <-- nav.value.toWeakSignal.map(_.getOrElse(1)).map{
        case 1 => mainContentArea
        case 2 => div(cls:="pt64",p("blah blah"))
      }
  )
